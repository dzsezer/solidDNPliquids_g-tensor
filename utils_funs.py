...
def iP4args(R1wI,R2wI,w1,Ws):
    '''
        Calculates the inverse of P given in Eq. (32) of the paper
    '''
...
def eprRMSDerror10(x,exp,calc):
    ...
    return 1e4*np.mean((y_pred-y_true)**2)


def eprRMSDerror16(x,exp,calc):
    y_true = exp.epr_y
...
def dnpRMSDerror(x,exp,calc):
    ...
    return 1e4*np.mean(y_true**2*(y_pred-y_true)**2)
